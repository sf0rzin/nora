...
if [ -f "$AGE_KEY_FILE" ]; then
  ...
  1. Paste it into ${PROXMOX_DIR}/.sops.yaml as a recipient.
  ...
  offline copy (see docs/operations/proxmox-deploy.md §chave-age).
...
cat <<EOF
  ...
  Next steps (docs/operations/proxmox-deploy.md):
    ...
    5. Restore from a backup, if needed             ->  scripts/restore-into-proxmox.sh
