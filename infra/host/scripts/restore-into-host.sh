#!/usr/bin/env bash
#
# restore-into-host.sh — restores a pg_dump (originally the one-time Azure migration dump,
# today the hourly on-host backup) into the local stack's containers.
#
...
usage() {
  cat <<EOF
$SCRIPT_NAME — restores a pg_dump into the postgres/postgres-platform containers
...
COMPOSE_ARGS=(--project-name "$COMPOSE_PROJECT" --project-directory "$HOST_DIR" -f "$COMPOSE_FILE")
...
# ---------------------------------------------------------------------------
hr
log "restore-into-host — compose project '$COMPOSE_PROJECT'"
