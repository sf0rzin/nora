...
export type PanelTab = "flow" | "executions";

const STATUS_EXEC: Record<WorkflowExecutionStatus, { label: string; color: string }> = {
  SUCCESS: { label: "Sucesso", color: "var(--success)" },
...
function FormDiscord({
  ...
  const invalida = mostrarErros && !isDiscordWebhook(webhookUrl);
...
function FormGitHub({
  ...
  const invalido = mostrarErros && !isGitHubRepo(repo);
...
function TabFluxo({ nos }: { nos: RFNode[] }) {
  const gatilho = nos.find((n) => n.data.kind === "trigger");
  const nomeGatilho = gatilho ? blockMeta(gatilho.data.blockType)?.name ?? "—" : null;
              ...
              >
                ...
                <span style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ display: "block", fontSize: 11, color: "var(--muted)" }}>
                    {blockMeta(ex.eventType)?.name ?? ex.eventType} · {relativeTime(ex.createdAt)}
                  </span>
                </span>
                <svg
                  width="12"
                  ...
                  {ex.log.length === 0 ? (
                    ...
                    ex.log.map((linha, i) => (
                      <div key={i} className={`flows-exec-linha${linha.level === "error" ? " is-err" : ""}`}>
                        <span className="hora">{logTime(linha.at)}</span>
...
export function SidePanel({
  ...
  if (no) {
    const meta = blockMeta(no.data.blockType);
    ...
    return (
      <aside className="flows-panel" aria-label="Parâmetros do nó">
        <div style={{ padding: "14px 14px 0" }}>
          ...
          >
            {meta?.Icon ? <meta.Icon /> : <KindIcon kind={no.data.kind} />}
            {kindMeta.label}
          </div>
          <h2
            style={{
    ...
    <aside className="flows-panel" aria-label="Painel do fluxo">
      <div className="flows-tabs" role="tablist">
        <button
          ...
          className={`flows-tab${tab === "flow" ? " is-active" : ""}`}
          onClick={() => onTab("flow")}
        >
          Fluxo
        </button>
        <button
          ...
          className={`flows-tab${tab === "executions" ? " is-active" : ""}`}
          disabled={!fluxoSalvo}
          title={fluxoSalvo ? undefined : "Salve o fluxo para ver execuções"}
          onClick={() => onTab("executions")}
        >
          Execuções
        </button>
      </div>
