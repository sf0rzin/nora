...
| `desktop-release.yml` | edited | Adjusted for the local STT of ADR 0035, which no longer exists: ADR 0039/0045 moved transcription to the provider's realtime API, so the `whisper-rs` C++ toolchain requirement and the `MAX_PATH` workaround that came with it were removed and the point of attention this row used to raise is closed. (The matrix was three platforms when this table was written; the client is Windows-only now.) |
