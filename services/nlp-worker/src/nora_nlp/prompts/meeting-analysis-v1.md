...
15. **`dueDate` é uma data ISO 8601 (`YYYY-MM-DD`) ou `null` — nunca uma expressão.** Reuniões falam em datas relativas ("até sexta-feira", "eu retorno amanhã", "na próxima terça"). Você não recebe a data da reunião, então não tem âncora para resolvê-las: nesses casos emita `null` e registre o compromisso no `title` e no `sourceQuote`. Só preencha `dueDate` quando a transcrição disser uma data absoluta ("dia 12 de setembro", "01/09/2026"). Escrever a expressão no campo é o erro que este item existe para evitar.
