...
- USE_LLM_STUB=false -> OpenRouter (LLM real com structured output)
"""
...
def analyze(req: AnalyzeRequest, settings: Settings = Depends(get_settings)) -> AnalyzeResponse:
    ...
    try:
        ...
        logger.error("Configuracao do OpenRouter invalida: %s", exc)
        raise HTTPException(
        ...
        logger.exception("Erro inesperado na chamada ao OpenRouter")
        raise HTTPException(
