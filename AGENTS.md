...
- **Web** is Next.js 16 + TypeScript + **raw Tailwind, no shadcn** (ADR 0013) with an OKLCH token palette (`apps/web/src/styles/tokens.css`) and **DM Sans + JetBrains Mono** loaded through `next/font/google` (design system v3, `apps/web/src/app/layout.tsx`; JetBrains Mono is reserved for code contexts). Its automated suite is **Vitest unit tests plus three Playwright e2e specs**, both run by the `web` job: the e2e specs cover security headers, route protection and CSP violations, and the unit suite (ADR 0042) covers seven `src/lib` modules — the `request()` function every one of `client.ts`'s 74 exported wrappers goes through, the Markdown report builder, the task-list CSV/Markdown exporter, the consolidated period report, the BFF's PII redaction, the password policy and the trends panel's date/axis helpers. **No page and no component has a unit test**, so whole-app coverage is around 8%; the coverage gate is per module and only on `redact.ts`, `markdown.ts`, `tasks-export.ts`, `usage-report.ts` and `password-policy.ts`
...
| **0041** — NORA as an MCP server | **Built** (US27, migration V029). An inbound adapter inside `services/api` — `POST /mcp`, JSON-RPC 2.0 over Streamable HTTP, five read tools — every call resolving a real IAM principal through `PolicyEvaluator` with **no MCP-specific permission vocabulary**, and a hashed tenant-scoped bearer token instead of a full OAuth 2.1 authorization server. ADR 0031's OAuth integrations are the **outbound** path; MCP is the **inbound** one. The invariant, now a test rather than an intention: an MCP client can never see more than the user it acts for can see in the web application. Two limits shipped on purpose — **read-only**, and not the specification's OAuth flow, so a client that speaks only that flow needs a pasted token |
...
| Date | Change |
|---|---|
| 2026-08-17 | **The tenant can now see its own AI consumption** (US33, `GET /usage`) and export the period (US34, client-side). Two things an agent must know before touching either. First, `usage_events` lives in the **control plane** (ADR 0022), which is off by default in local, test and CI — so the AI half of that endpoint answers `ai.state: UNAVAILABLE` on a developer machine, and that is correct behaviour, not a broken fixture. Second, the operator aggregation next to it (`UsageEventRepository.aggregate`, US83) **has no tenant parameter** and must never be reused to answer a tenant; `tenantSeries` exists precisely so no code path can forget the predicate, and RLS would not catch the mistake because the platform database has none. The two web bullets moved again: **seven** `src/lib` modules, `client.ts` at **74** exported wrappers and **five** gated files. The `69` this file, `apps/web/README.md` and `vitest.config.mts` all carried was already stale by four before this change — the row below says it was re-measured for US21, and it was not |
| 2026-08-17 | **The MCP server was built** (US27, ADR 0041, migration V029), so the ADR 0041 row of the scope table stops saying "nothing is built" and says what exists, at the size it exists: inbound only, read-only, and deliberately not the specification's OAuth 2.1 authorization server. Same correction applied wherever else the repository asserted the absence in the present tense — `docs/product/vision.md` (§1, §2, §3, §4, §5, §7, §11), `docs/product/backlog.md` (E6 and US27), `docs/product/roadmap.md` (§2.3 and the prerequisites note), `docs/engineering/architecture.md` (§1, §12, the debt list, and a new §20 with the inbound flow), `docs/engineering/standards.md` (§2 and §3) and `.github/copilot-instructions.md`. The sidebar's connector entry also stopped labelling itself MCP: that page is ADR 0031's OAuth, the outbound direction |
