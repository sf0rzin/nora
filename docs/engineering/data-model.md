...
### 2.43 `iam_permission_boundaries` — V033
...
Listed in ADR 0006 and/or the old `data-model.md`, but **with no corresponding migration** (V001–V033 do not cover them; inventory in §5).
...
### RLS — Row-Level Security (V016 → V017 → V019 → V020 → V021 → V022 → V023 → V024 → V028 → V033 → V032; V029 exempt)
...
- `CREATE POLICY tenant_isolation` + `ENABLE ROW LEVEL SECURITY` defined on tenant-owned tables that have their own `tenant_id`:
  ...
  - **V033 (1, policy defined and inert):** `iam_permission_boundaries` (permission boundaries, US44) — an **IAM authorization** table, so it is created the way V020 left its family: policy defined, RLS **not** enabled. Counted here because the policy exists; it is not counted as an enforced table.
...
| **V019** | full RLS (ADR 0026): `ENABLE RLS` + `tenant_isolation` policy on the remaining 15 tenant-owned tables (priority `transcripts` = PII), closing the coverage started in V016/V017 (28 tables with a direct policy through V019; +1 in V021, +5 in V022–V024, +1 in V028, +1 in V033, +1 in V032 → **37**). Cascade boundaries documented (no policy). Role provisioning versioned in `db/operational/R001` (admin) |
