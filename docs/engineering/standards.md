...
The current version covers EMAIL, PHONE, CPF, CNPJ, CREDIT_CARD **and** PERSON_NAME (heuristics + a BR list of ~270 names + a negative list of ~80 terms) — the six values in `packages/shared-contracts/pii-types.json`. **ADDRESS is not implemented**, and it is declared debt rather than closed scope (ADR 0040 Consequences; `docs/product/vision.md`).
