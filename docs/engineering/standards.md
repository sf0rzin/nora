...
- **Take the next free number, and expect it to be the highest one.** `V031` is a permanent gap and the reason is worth reading before anyone tries to fill it: US41 reserved it, then shipped policy templates as a code catalogue and never wrote the file, and `V032` took the next number while it sat empty. US44 first filled the hole — which produced an **out-of-order** migration, where a database that had already applied V032 refuses to boot because Flyway finds a lower version pending. The fix was not `spring.flyway.out-of-order`, which would have loosened ordering for every future migration to work around one numbering mistake; it was to renumber the unreleased file to `V033`. **A reserved number that goes unused stays unused.**
- See `docs/engineering/data-model.md` for the full migration map (**V001–V033**; highlights since V017: V017 Customer Confidence, V018 invitation token hash, V019/V020 full RLS + auth-aware scope, V021 `meeting_embeddings`, V022 chat sessions, V023 Flows, V024–V026 OAuth integration connections and the provider CHECK growing to nine, V027 the composite IAM FK, V028 the company-context history, V029 the inbound MCP credential, V030 the trends panel's completion timestamp, V033 the per-user permission boundary, V032 the run state of the scheduled Flows trigger). Single source of truth for the schema. The control plane has its own separate line under `services/api/src/main/resources/db/platform/` (ADR 0022).
