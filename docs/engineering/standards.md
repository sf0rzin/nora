...
- See `docs/engineering/data-model.md` for the full migration map (**V001–V028**; highlights since V017: V017 Customer Confidence, V018 invitation token hash, V019/V020 full RLS + auth-aware scope, V021 `meeting_embeddings`, V022 chat sessions, V023 Flows, V024–V026 OAuth integration connections and the provider CHECK growing to nine, V027 the composite IAM FK, V028 the company-context history). Single source of truth for the schema. The control plane has its own separate line under `services/api/src/main/resources/db/platform/` (ADR 0022).
