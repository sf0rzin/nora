...
- **Take the next free number, and expect it to be the highest one.** `V031` is the single exception: US41 reserved it, shipped without a table and released it, V032 took the next number, and US44 later filled the gap rather than renumbering a migration that may already have run. That made V031 **out of order**, which is why `spring.flyway.out-of-order` is `true` in `application.yml` — without it a database holding V032 fails `validate` on boot with "Detected resolved migration not applied to database: 031". The flag makes a *genuinely* accidental out-of-order file boot silently too, so the check is the inventory in `data-model.md` §5, not the runtime.
- See `docs/engineering/data-model.md` for the full migration map (**V001–V032**; highlights since V017: V017 Customer Confidence, V018 invitation token hash, V019/V020 full RLS + auth-aware scope, V021 `meeting_embeddings`, V022 chat sessions, V023 Flows, V024–V026 OAuth integration connections and the provider CHECK growing to nine, V027 the composite IAM FK, V028 the company-context history, V029 the inbound MCP credential, V030 the trends panel's completion timestamp, V031 the per-user permission boundary, V032 the run state of the scheduled Flows trigger). Single source of truth for the schema. The control plane has its own separate line under `services/api/src/main/resources/db/platform/` (ADR 0022).
