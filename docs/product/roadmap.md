...
- **Spring backend**: **77.1-77.3%** instruction / **61.5-61.6%** branch / 78.0-78.1% line (578 tests) — *measured 2026-08-17*. The areas ADR 0018 names critical are above their target: IAM packages **90.9%** instruction, Auth/identity packages **93.8%**, `PolicyEvaluator` **96.3%** instruction / 86.0% branch
...
> **Status update (2026-08-17):** items **(e)** and **(f)** above are delivered — see the 1.11 row of the table below for the files. The "tension with the dataset's Hardcode Policy" the note of 2026-05-23 records is resolved and written down rather than left implicit: the rule constrains product code, not demonstration data, and the seeded vendor is fictional for reasons of its own. Of the six items, only **(d)** remains, and it is subjective by construction.

| Sub-phase | Target window | Scope | Prerequisites |
|---|---|---|---|
| **1.11 — Demo Polish Plan A** | 2-3 agentic weeks (target: close with buffer before the pitch) | (a) **Done (#148)** — **Minimum Customer Confidence** via ADR 0015: schema → persistence (V017) → read-only endpoint `GET /meetings/{id}` → `CustomerConfidenceCard` in MeetingDetail<br>(b) **Done (1.11b)** — **`AUTH_FILTER_HARD_CAP` fix**: silent ceiling of 500 removed; `MeetingService.listAllForAuthFilter` scans in batches before the IAM filter. SQL pushdown via JSONB+GIN remains a future performance optimisation<br>(c) **Done (1.11c)** — **`PolicyEvaluator` expansion**: `StringIn`, `StringLike`, `DateGreaterThan`, `DateLessThan` implemented (fail-closed retained for an unknown operator and a missing attribute)<br>(d) **Polished internal UX**: editorial v3 on the auth pages, fix of the improvised `position:fixed` on login, fine adjustments in MeetingDetail and the dashboard<br>(e) **Done (2026-08-17)** — **Demonstration seed**: [`scripts/seed-demo.sh`](../../scripts/seed-demo.sh) (`make seed-demo`) populates a local environment over the HTTP API — never by direct SQL — with the Core workspace (Lucas), the Enterprise workspace (Camila as root, a scoped IAM policy, an invitation for Rafael), 7 analysed meetings and one flow. The synthetic material is the `meridian-erp` tenant plus meetings 13-17 in [`data/synthetic/`](../../data/synthetic/README.md), including a 3-meeting arc against one account so Customer Confidence has a trend. The vendor is fictional rather than TOTVS; `data/synthetic/README.md` records the reasoning, and why a vendor-vocabulary seed would not have violated the no-hardcode rule either way<br>(f) **Done (2026-08-17)** — **Demo script**: [`docs/challenge/demo-script.md`](../challenge/demo-script.md), with a per-block running time, a plan B per block, and the two things the demo cannot honestly show today (Customer Confidence under `USE_LLM_STUB=true`; live desktop transcription, mid-migration per ADR 0039). **The duration lives in that file and only there** — this cell used to state a running time, `fiap-challenge-2026.md` stated a different one, and neither described a script that existed | Sub-phase 1.10 closed (docs refresh consolidated); ADR 0015 created and approved |
...
| **Polish + Demo + Pitch** | Partially DONE | The seed and the demo script are delivered — [`scripts/seed-demo.sh`](../../scripts/seed-demo.sh) and [`docs/challenge/demo-script.md`](../challenge/demo-script.md). UX polish (item 1.11d) is subjective and stays open |
| **Trends panel** (US21) | MISSING / C | Prerequisite US15 (semantic search) already delivered — provider-agnostic embeddings + HTTP embedding client (PR #206, migration V021). **Not pgvector:** V021 stores the vector as a JSON array in a `TEXT` column and `EmbeddingService` computes cosine similarity in Java; the migration's own header says pgvector is the future optimisation, not the current mechanism. The panel's own temporal analysis is still missing |
...
| 1.2 | 2026-08-17 | Cumulative metrics: coverage re-measured in CI after three months of "to be re-measured". Backend 77.1-77.3% instruction / 61.5-61.6% branch (was 67% / 53%; the range is measured run-to-run jitter, not rounding), worker 92.4% (was 87%), web restated as unmeasured rather than 0%. `scripts/report-coverage.sh` now publishes all of it on every run, so the figures stop being a snapshot |
| 1.3 | 2026-08-17 | Sub-phase 1.11 items (e) seed and (f) demo script recorded as delivered, pointing at `scripts/seed-demo.sh` and `docs/challenge/demo-script.md`. The demo duration stops being repeated here and lives in the script alone. The "semantic search via pgvector" claim corrected against migration V021 |
