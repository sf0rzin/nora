...
> Roadmap **vivo e oficial** do NORA. Substitui o antigo `docs/plano-de-execucao.md` (arquivado — descontinuado em 2026-05-14, pois descrevia plano semana-a-semana entre Anthony + colega que já não corresponde ao fluxo real: hoje Anthony executa sozinho com Claude Code subagentes em paralelo via worktrees).
>
> **Estrutura:**
...
> 3. **Fase pós-MVP (longo prazo)** — visão das fases 4-9 do plano de carreira original
...
| **1.10 — Docs Refresh** (em curso) | 2026-05-13/14 | em progresso | Audit `2026-05-13-audit-pre-subfase-1.10.md` (13 seções) + reorganização `docs/` em `product/` + `engineering/` + `operations/` + `security/` + `challenge/`. Decisões: ADR 0013 a criar (status oficial Customer Confidence) · ADR 0014 (defer 14 US pós-MVP) já decidida em bloco · ADR 0015 a criar na 1.11 (escolha Customer Confidence mínimo vs remove) | (ADR 0013, 0014 a criar) |

> Customer Confidence (US48-51) e tabelas correspondentes **foram adiadas em bloco** via decisão de Anthony em 2026-05-14 (ADR 0014). Não tem ADR sucessor explicando ainda — vai entrar como ADR 0013 na 1.10 (status oficial) + ADR 0015 na 1.11 (escolha de implementação mínima vs landing-only).

### Métricas acumuladas

- **75+ PRs** mergeados em `main`
- **12 ADRs** aceitos (0001-0012)
...
> Visão das fases 4-9 do plano de carreira original do Anthony. Várias dessas fases já foram **absorvidas pelas sub-fases 1.X** ou **viraram Won't Have v1 com critério de reativação**. Esta seção mantém a visão de produto longo-prazo, sem ser plano de execução.
...
|---|---|---|
| 1.0 | 2026-05-14 | **Criação inicial** como roadmap vivo. Substitui `docs/plano-de-execucao.md` (descontinuado — descrevia plano semana-a-semana entre Anthony + colega, fora do fluxo real atual). Consolida histórico das 11 Sub-fases (1.0-1.10) com cross-check do audit `2026-05-13-audit-pre-subfase-1.10.md` §11. Define próximas Sub-fases 1.11 (Demo Polish Plano A), 1.12 (Production Hardening), 1.13+ (pós-pitch) com escopo e pré-requisitos explícitos. Inclui visão longo-prazo (3 horizontes H1-H3) e notas de processo |
