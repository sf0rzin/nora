...
| **1.5 — Briefing Desktop** | 2026-05-12 | (sem PR — vault) | Vault `40-desktop-handoff/2026-05-12-update-pos-subfase-1.4.md`. Briefing para o amigo dono do desktop com contratos NDJSON Rust↔Python + roadmap macOS BlackHole (já mergeado em PR #37) + débito ScreenCaptureKit | — |
...
> **Atualização (reconciliação 2026-05-21, pós-PR #148):** (1) **Customer Confidence (US48-49) FOI implementado** full-stack em #148: V017 + worker emit + `AnalysisService` wiring (trend server-side) + `GET /meetings/{id}` + `CustomerConfidenceCard`. Dívida narrativa da landing **resolvida**. (2) Foi entregue como **V017** (o slot V013 do ADR 0015 acabou em soft-delete, #114). (3) **RLS**: schema entregue em V016 e completado em V019/V020 (RLS completa + scope auth-aware) — resta só o cutover/enforcement operacional em prod (ADR 0026/0028). (4) **A Sub-fase 1.11 já foi entregue** nos itens de código: (a) Customer Confidence DONE; (b) AUTH_FILTER_HARD_CAP removido; (c) PolicyEvaluator operators implementados; restam (e) seed TOTVS e (f) roteiro de demo. Account Health agregado (US50-US51) **continua deferido** via ADR 0014.

### Métricas acumuladas

- **200+ PRs** mergeados em `main` (último: #206 Chat RAG em 2026-06-05; inclui a onda de hardening "audit follow-up" #114–#138 pós-1.10)
- **ADRs**: ver o índice canônico em `docs/adr/README.md` (fonte única de verdade da contagem e do status). ADR 0006 substituído parcialmente por 0015; o hardening pós-1.10 foi documentado retroativamente em ADR 0019 (tenant isolation defense-in-depth), 0020 (token rotation) e 0021 (soft-delete)
- **Migrations**: schema atual descrito em `docs/engineering/data-model.md` (fonte única). Marcos recentes: V016 RLS (schema), V017 Customer Confidence, V018 hash do token de convite, V019/V020 RLS completa + scope auth-aware, V021 `meeting_embeddings`
...
- **Web Next.js**: 0% (sem runner; débito para 1.12)
...
> Janelas alvo são **agentic** (modelos Opus para arquitetura/segurança/dados e modelos Sonnet para implementação focada/UI/testes, em paralelo via worktrees), não human-hours. Refletem complexidade real de cada slice, não esforço humano.

> **Status real (2026-05-23, reconciliado):** a **Sub-fase 1.11 está majoritariamente entregue** (~3 de 6 itens de código): **(a) Customer Confidence** (#148), **(b) AUTH_FILTER_HARD_CAP** (teto silencioso de 500 removido — scan em lotes em `MeetingService.listAllForAuthFilter`) e **(c) PolicyEvaluator operators** (`StringIn`/`StringLike`/`DateGreaterThan`/`DateLessThan`, fail-closed mantido) — Concluído. Faltam: **(e) seed TOTVS realista** (sintéticos atuais são acme/northwind genéricos — e há tensão com a "Política de Hardcode" do dataset, que proíbe tenant TOTVS), **(f) roteiro de demo**. (d) UX polish é parcial/subjetivo. Da **Sub-fase 1.12**, o item (b) **RLS já foi entregue antecipadamente** (schema em V016, completado em V019/V020).

| Sub-fase | Janela alvo | Escopo | Arquiteto responsável | Pré-requisitos |
|---|---|---|---|---|
| **1.11 — Demo Polish Plano A** | 2-3 semanas agentic (alvo: fechar com buffer antes do pitch) | (a) **Concluído (#148)** — **Customer Confidence mínimo** via ADR 0015: schema → persistência (V017) → endpoint read-only `GET /meetings/{id}` → `CustomerConfidenceCard` no MeetingDetail<br>(b) **Concluído (1.11b)** — **`AUTH_FILTER_HARD_CAP` fix**: teto silencioso de 500 removido; `MeetingService.listAllForAuthFilter` varre em lotes antes do filtro IAM. Pushdown SQL via JSONB+GIN fica como otimização de performance futura<br>(c) **Concluído (1.11c)** — **`PolicyEvaluator` expansion**: `StringIn`, `StringLike`, `DateGreaterThan`, `DateLessThan` implementados (fail-closed mantido para operador desconhecido e atributo ausente)<br>(d) **UX interna polida** (Arquiteto Design): editorial v3 nas auth pages, correção do `position:fixed` improvisado no login, ajustes finos no MeetingDetail e dashboard<br>(e) **Seed sintético TOTVS realista**: 5-7 reuniões com vocabulário TOTVS (Protheus, RM, Datasul, Fluig, RM Consult) + 3 tenants demo (1 com Customer Confidence ativado) + Camila/Rafael/Lucas users + policies de exemplo<br>(f) **Roteiro de demo**: script de 8-10 minutos cobrindo Core (Lucas faz upload, vê resumo, marca task) → Enterprise (Camila configura policy, Rafael vê só seu escopo) → Customer Confidence (Rafael vê signals do lead). Inclui plano B se algo falhar ao vivo | Joint Tech Lead + Design | Sub-fase 1.10 fechada (docs refresh consolidado); ADR 0015 criado e aprovado |
| **1.12 — Production Hardening** | 1-2 semanas agentic | (a) **`rg-nora-prod` separado** do `rg-nora-dev` (isolamento total: KV, Postgres, Storage, ACA env distintos)<br>(b) **RLS Postgres** — schema entregue em **V016** e completado em **V019/V020** (RLS completa + scope auth-aware); resta só o cutover/enforcement operacional em prod (role `nora_app` `NOBYPASSRLS` + flag `nora.security.rls.enforce`), conforme runbook de cutover (ADR 0026/0028)<br>(c) **Monitoring alerts** Azure Monitor: P95 latency, 5xx rate, Postgres CPU/conn pool, KV access failures, Speech token exhaustion<br>(d) **LGPD operacional** — **Concluído (ADR 0029)**: `DELETE /privacy/meetings/{id}` (direito ao esquecimento) + `RetentionSweeper` agendado + `PrivacyFlowIntegrationTest`. Resta evoluir a tabela `audit_events` global (não só IAM) e a retenção declarada por tenant<br>(e) **DR runbook** (`docs/operations/dr-runbook.md`): backup Postgres + restore drill + RTO/RPO declarado + Bicep redeploy a partir de zero<br>(f) **Secrets rotation**: política de rotação JWT_SECRET, OPENAI_API_KEY, ConnectionString Postgres via KV versions + redeploy automático<br>(g) **Test coverage targets** (ADR 0018 a criar): >85% áreas críticas (IAM, Auth, PII, LLM analyzer), >60% demais, >50% web nas pages principais, >70% branch coverage backend. Adicionar Vitest no web | Tech Lead | Sub-fase 1.11 fechada; ADR 0016 (estratégia rg-prod) e ADR 0018 (coverage targets) a criar |
| **1.13+ — Pós-pitch (15/06 em diante)** | Depende do desfecho do Plano A | **Cenário A — Plano A move (TOTVS sinaliza interesse):** dossier de pitch técnico-comercial · due-diligence material (SECURITY.md robusto, threat-model STRIDE, LGPD checklist completo, cost projection multi-tenant) · suporte a 1ª reunião comercial · roadmap de POC contratada<br>**Cenário B — Plano A neutro/negativo:** Plano C content first (post LinkedIn cobrindo NORA + 8 armadilhas Azure + IAM AWS-style + Productivity Score; thread Twitter; case no portfolio) + Plano B pivô comercial (criar landing com pre-order, definir pricing pilot >= R$300/tenant/mês baseado em unit economics audit §13, identificar 3-5 leads B2B fora TOTVS) | Tech Lead (+ Stratfy PO) | Pitch FIAP/TOTVS realizado 15/06/2026 |

### Critérios de "Sub-fase fechada"

Para uma sub-fase ser considerada **fechada** (`DONE`):
...
5. Roadmap atualizado movendo a sub-fase de "Próximas" para "Histórico"
...
| **Customer Confidence completo** (US48-US51) | **DONE parcial** | US48-US49 entregues full-stack em #148 (schema V017 + worker emit + trend autoritativo por conta + `GET /meetings/{id}` + `CustomerConfidenceCard`). Account Health agregado + alertas (US50-US51) = pós-pilot quando 3+ tenants tiverem >10 reuniões cada |
...
| **MCPs (Calendar, Tasks, CRM)** (US27-US29, US47) | MISSING / W | Reativar quando primeiro tenant Enterprise pago pedir integração concreta. Seguem deferidos como conceito de roadmap (sem implementação no monorepo) |
...
| **Polimento + Demo + Pitch** | em curso na Sub-fase 1.11 | Sub-fase 1.11 (Demo Polish Plano A) cobre isso para o pitch 15/06 |
| **Painel de tendências** (US21) | MISSING / C | Pré-requisito US15 (busca semântica) já entregue — embeddings provider-agnósticos via pgvector + HTTP embedding client (PR #206, migration V021). Falta a análise temporal própria do painel |
| **Templates de policy + Simulator** (US41 + US43) | MISSING / S | US43 (simulator) provavelmente sobe na 1.11 — sem isso, debug de policies é cego para o primeiro pilot pago. US41 (templates) só quando >3 tenants pedirem onboarding rápido |
...
1. **Horizonte H1 (hoje → 15/06/2026 pitch)**: validação Plano A com TOTVS via demo polida + Customer Confidence visível + IAM enterprise-grade
...
- **Customer Confidence completo** depende de retenção/LGPD antes de virar pago (Account Health tem implicações de retenção). A base de LGPD operacional já foi entregue via ADR 0029 (`DELETE /privacy/meetings/{id}` + `RetentionSweeper`)
- **MCPs** dependem da evolução do schema de contratos compartilhados (`packages/shared-contracts/`), que hoje já contém `error-codes.md`, `pii-types.json`, `processing-status.json` e `README.md`
