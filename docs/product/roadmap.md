...
| **STT decided twice, then migrated** | 2026-08-07 → 2026-08-17 | — | ADR 0035 moved transcription onto the client (Whisper in Tauri) when the Azure Speech resource died; ADR 0039 superseded it on 2026-08-16 and moved transcription to OpenAI's streaming API behind an ephemeral session token; ADR 0045 fixed the contract and the migration landed on 2026-08-17. The on-device engine, its `whisper-rs` C++ build and the CMake/`MAX_PATH` workarounds in both desktop workflows are out of the tree | ADR 0035, 0039, 0045 |
...
- ~~**Open, with no scheduled trigger**~~ — **this category is empty, and emptying it is what ADR
  ...
  ADR 0046 §1 reactivated US33, US34, US41 and US44, and all four are delivered — US33 and US34 as
  `GET /usage` plus a client-side period export, US41 and US42 as a code-shipped template catalogue
  and a form beside the JSON editor, US44 as a permission boundary on one user (migration V033,
...
| **Policy templates + Simulator + form editor** (US41 + US42 + US43) | All three **delivered**. US43 was reactivated by ADR 0038 §5 — `POST /iam/simulate`; US41 and US42 by ADR 0046 §1, as `GET /iam/policy-templates` and a form beside the Monaco editor. US41 took **no migration**: the templates are a catalogue shipped in code, because a flagged row in `iam_policies` would still be an attachable policy id. The number it had reserved, V033, was later taken by US44 |
| **Permission boundaries** (US44) | **Reactivated** by ADR 0046 §1 and **delivered** — `GET`/`PUT`/`DELETE /iam/users/{userId}/boundary`, migration V033, decisions in ADR 0049. The old note here said it "needs an organisational hierarchy and IAM delegation that nothing else asks for", and that half stayed true: what shipped is a cap on **one user**, with no hierarchy, no group boundaries and no service control policies. Two limits are written into the ADR rather than discovered later — the tenant **Root is not capped** (a boundary on it would have no key, and there is no support desk), and AWS's `iam:PermissionsBoundary` condition key is not built, so a delegated admin can still create an unbounded user |
...
- **The trends panel** was believed to depend on the embeddings backfill having been *run* for the
  ...
  analysis writes, so a half-filled RAG index does not affect it at all — the prerequisite that was
  real is much smaller: a tenant needs meetings that have actually been *analysed*, and the panel
  says which of "no meetings", "none analysed" and "no activity" it is looking at
