...
| **Pós-1.10 — Hardening (audit follow-ups)** | 2026-05-15+ | #114–#138 | Onda de segurança/infra rotulada "audit follow-up #N", **fora do histórico de sub-fases nomeadas**: V013 soft-delete (`deleted_at` + `@SQLRestriction`) · V014 refresh-token rotation + reuse-detection · V015 composite FK de isolamento · **V016 Row-Level Security** (+ `TenantRlsAspect`) · JWT RS256 + JWKS · audit log de auth expandido · App Insights Java agent · upload hardening. **Nenhum tem ADR dedicado (débito).** | — (recomendados: ADR RLS/isolamento, ADR token rotation, ADR soft-delete) |

> **⚠️ Drift de planejamento (auditoria 2026-05-21):** (1) **Customer Confidence (US48-49) NÃO foi implementado** — ADR 0015 está "aceito" mas o worker não emite `customerConfidence` e não há migration/endpoint/UI; a dívida narrativa da landing segue aberta. (2) O slot **V013, que o ADR 0015/roadmap reservaram para `customer_confidence`, foi usado para soft-delete**. (3) **RLS (item 1.12b abaixo) já foi entregue** no schema via V016 — falta só o enforcement em prod. (4) **A Sub-fase 1.11 não foi iniciada** (ver §2). Account Health agregado (US50-US51) **continua deferido** via ADR 0014.

### Métricas acumuladas

- **~144 PRs** mergeados em `main` (last visto: #144 em 2026-05-21; inclui a onda de hardening "audit follow-up" #114–#138 pós-1.10)
- **18 ADRs** aceitos/propostos (0001-0018; ADR 0006 substituído parcialmente por 0015; ADR 0013 e 0016 propostos). Ver `docs/adr/README.md`. **Débito:** o hardening pós-1.10 (RLS, soft-delete, token rotation, composite FK) entrou sem ADR — criar (auditoria 2026-05-21)
- **Migrations**: V001–**V016** (V013 soft-delete, V014 refresh rotation, V015 composite FK, V016 RLS)
...
> **Status real (2026-05-21):** a **Sub-fase 1.11 ainda NÃO foi iniciada** (0 de 6 itens; sem branch/commit) — a ~3 semanas do pitch (12/06) e já passado o alvo de fechamento (05/06). Itens (a) Customer Confidence, (b) AUTH_FILTER_HARD_CAP (ainda `=500` em `MeetingsController.java:67`), (c) PolicyEvaluator operators (ainda só `StringEquals` em `:37`) seguem abertos. Da **Sub-fase 1.12**, o item (b) **RLS já foi entregue antecipadamente** (V016). A energia pós-1.10 foi para a onda de hardening (#114–#138, ver §1), não para o escopo planejado de 1.11. **Recalibrar com o PO.**
