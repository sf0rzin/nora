...
**Account Health Score** — Score aggregated per **account** (not per meeting), expressing the temporal health of the relationship with an Enterprise customer/lead. Scale 0-100 with bands `AT_RISK` / `WATCH` / `HEALTHY` / `STRONG`. Computed from Customer Confidence + risks + opportunities. ADR 0006 accepted; **closed scope since 2026-08-16 (ADR 0038 §4)** — US50 and US51 are `WONT`, not deferred. They aggregate over history that does not exist, and `account_health_snapshots` was foreseen in the data model and never migrated. The old reactivation criterion ("post-pilot with 3+ tenants") came from ADR 0014, which ADR 0038 supersedes. The per-meeting signal that *is* built is **Customer Confidence**.
...
**RAG** — Retrieval-Augmented Generation. AI pattern where the LLM prompt is enriched with relevant documents retrieved from a knowledge base. In NORA, used to bring **tenant context** (products, glossary, competitors, stakeholders) into the prompt, plus semantic search over meetings. **Delivered (US15, PR #206)**: provider-agnostic embeddings (Gemini/OpenAI) persisted by migration V021 and retrieved by an HTTP embedding client (`EmbeddingService.java`, `HttpEmbeddingClient.java`). The Core chat consumes `/meetings/search` as RAG context. **It does not use `pgvector`**, despite this entry having said so: V021 stores the vector as a JSON array in a `TEXT` column and `EmbeddingService.cosine` computes the similarity in Java. The extension is available in the `pgvector/pgvector:pg16` image and is deliberately never created (ADR 0034 §excluded scope). (It does not use Azure AI Search either.)
...
**rg-nora-prod** — Azure production Resource Group. **Never existed, and is not planned.** It was scope of a "Sub-phase 1.12 — Production Hardening" that this glossary and the roadmap both described until 2026-08-17; Azure was shut down on 2026-08-07 (ADR 0034/0036) and the plan was deleted rather than rescheduled. The current position on production readiness is ADR 0038 §6.

**RLS** — Row-Level Security. Postgres feature that filters rows by a SQL policy. **Schema delivered in V016** (`tenant_isolation` on 10 tables; predicate `tenant_id = nora.current_tenant_id()`, reading the GUC `nora.current_tenant_id` set by the `TenantRlsAspect`), with **full RLS + auth-aware scope in V019/V020** and a cutover runbook (ADR 0026/0028). The cutover was carried out: RLS is **enforced on the deployed stack since 2026-08-10** — the API connects as `nora_app` (NOBYPASSRLS), the operator aggregate reads through `nora_telemetry` (BYPASSRLS), and `RlsEnforceTelemetryGuard` refuses to boot on a half-applied cutover. It is **off by default in the repository**, where the application filter is the only control; flipping that default is a declared deferral (ADR 0038 §6g). Defense in depth for the app filter (ADR 0002). (Note: the real GUC is `nora.current_tenant_id`, not `app.tenant_id` as ADR 0002 sketched.)
...
**V001 - V027** — Current Flyway migrations (canonical source: `docs/engineering/data-model.md`). Each one idempotent and immutable once merged, sequentially numbered — with one recorded exception, V027 (see the **Flyway** entry). Highlights: V013 = soft-delete, V014 = refresh-token rotation, V015 = composite isolation FK, V016 = Row-Level Security (schema), V018 = invitation token hash, V019/V020 = full RLS + auth-aware scope, V021 = `meeting_embeddings` (JSON vector in a `TEXT` column, **not** pgvector — see the **RAG** entry), V022 = chat sessions, V023 = workflows, V024-V026 = integration connections, V027 = composite FK on the IAM attachment tables. Customer Confidence is persisted (see the **Customer Confidence** entry).
...
| 1.2 | 2026-08-17 | Five entries corrected while the roadmap was rewritten. **rg-nora-prod**: never existed and is not planned — the "Sub-phase 1.12" that would have created it was deleted with Azure (ADR 0034/0036), and ADR 0038 §6 is the current position on production readiness. **Account Health Score**: closed scope (`WONT`) by ADR 0038 §4, not deferred under ADR 0014's criterion. **RAG** and **V001-V027**: semantic search does **not** use pgvector — V021 stores a JSON array in a `TEXT` column and the cosine runs in Java — and the migration list stopped at V021 while the schema had reached V027. **RLS**: the enforce cutover this entry listed as outstanding was done on 2026-08-10; what is deferred is the repository default (ADR 0038 §6g) |
