...
- Coverage, measured by CI on 2026-08-17: NLP worker 92.4% statement (863 tests), Spring backend 77.1-77.3% instruction / 61.5-61.6% branch (578 tests), `apps/web` unmeasured — it has a Playwright e2e suite but no coverage instrumentation. Current figures: `scripts/report-coverage.sh`, published by the `api` and `worker` jobs on every run
...
| **Desktop** | A **real-time app** (Tauri 2 / Rust, on-device transcription) that captures and analyses the meeting while it happens, cross-platform | A videoconference plugin or a browser extension |
...
|  | **Does** | **Does Not** |
|---|---|---|
| **Input** | Accepts transcripts in **text** (`.txt`, `.vtt`, `.srt`) — MVP. Desktop captures system audio in **real time** (Windows via WASAPI loopback) | Does not accept upload of archived audio/video in the MVP (`.mp3`, `.mp4`) — it is in US08 (Won't Have v1). Roadmap: post-MVP via Azure Speech batch |
...
| **Desktop** | **Tauri 2 app** (Rust, on-device transcription) with system audio capture, **Windows only, via WASAPI loopback** | It is not a videoconference plugin. It does not run on macOS, on Linux or on mobile: the macOS (BlackHole) and Linux (PulseAudio) capture paths existed in the code but had never been exercised, and were removed rather than kept as a promise |
