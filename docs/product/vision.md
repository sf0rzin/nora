...
| **PII Shield** | **Automatically detects and redacts, in text**, CPF, CNPJ, confidential monetary values, email, phone, credit card, **Brazilian personal names (PERSON_NAME)** and **Brazilian street addresses (ADDRESS)** with `[[TIPO_N]]` placeholders, before submission to the analysis LLM (ADR 0012, ADR 0043). On the chat and semantic-search paths, structured PII is redacted in the BFF before the chat and embedding providers (ADR 0033) | **The promise is scoped to text and analysis** (ADR 0040). **ADDRESS is a deterministic street-type recogniser, not NER** (ADR 0043): an address whose name is lower-cased or purely numeric is out of scope, and it is not mirrored in the BFF. **The shield measures its own failure rate** — a corpus of ~5,900 cases gates a leak rate and a false-redaction rate, and both are published rather than claimed. **PERSON_NAME on the chat path is accepted residue** until worker routing lands (ADR 0033). **Audio is not redacted and cannot be**: redaction needs text, and text is what the transcriber produces — see the **Transcription** row |
