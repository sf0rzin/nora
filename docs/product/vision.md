...
| **Desktop** | **Tauri 2 app** (Rust, on-device transcription) with system audio capture. **Windows via WASAPI** (official v1) · **macOS via BlackHole** (virtual audio driver; native ScreenCaptureKit is in debt as a nice-to-have) · **Linux via PulseAudio** | It is not a videoconference plugin. It does not run on mobile in the MVP |
