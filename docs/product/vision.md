...
| **IAM** | A **granular access control** system in the style of **AWS IAM** (Root + Users + Groups + Policies created by the tenant itself, and a permission boundary that caps one user without granting anything) | A system where everyone in the company sees every transcript from every department |
...
| **PII Shield** | **Automatically detects and redacts, in text**, CPF, CNPJ, confidential monetary values, email, phone, credit card, **Brazilian personal names (PERSON_NAME)** and **Brazilian street addresses (ADDRESS)** with `[[TIPO_N]]` placeholders, before submission to the analysis LLM (ADR 0012, ADR 0042). On the chat and semantic-search paths, structured PII is redacted in the BFF before the chat and embedding providers (ADR 0033) | **The promise is scoped to text and analysis** (ADR 0040). **ADDRESS is a deterministic street-type recogniser, not NER** (ADR 0042): an address whose name is lower-cased or purely numeric is out of scope, and it is not mirrored in the BFF. **The shield measures its own failure rate** — a corpus of ~5,900 cases gates a leak rate and a false-redaction rate, and both are published rather than claimed. **PERSON_NAME on the chat path is accepted residue** until worker routing lands (ADR 0033). **Audio is not redacted and cannot be**: redaction needs text, and text is what the transcriber produces — see the **Transcription** row |
| **Transcription** | The Desktop streams audio to OpenAI's realtime API using an **ephemeral session credential** minted by the backend (ADR 0039, contract in ADR 0045). The provider account key never leaves the server, and the audio goes **desktop → provider directly**, never through NORA's infrastructure | **The audio does leave the user's machine, unredacted.** The earlier on-device arrangement (ADR 0035) kept it local; that was reversed knowingly. The transcription provider is a **declared external subprocessor** receiving raw audio before any redaction exists (ADR 0040 §3), there is **no data processing agreement** with it (ADR 0040 §5), and the honest name for the position is a **demonstration posture, not a compliance posture**. Because NORA never carries the audio, per-tenant usage is counted at **session issuance** — the cost figures are estimates, not measurements |
...
| **Core — Flows** | **Automations on a canvas** (ADR 0030/0032): a trigger, optional conditions, and actions on connected tools. All four real triggers are offered — `meeting.analysis_completed`, `action_item.created`, `meeting.risk_detected`, and `schedule.cron`, which a timer fires (US75, ADR 0047) | The schedule is a **closed vocabulary** — hourly, daily or weekly, in America/Sao_Paulo — not a cron expression, and nothing outside it can be saved. It is **not a digest**: a scheduled run executes once per meeting analysed since the previous run, so there is no "summary of the week" block. `schedule.cron` was rejected at save time until it had a dispatcher, rather than accepted and silently never fired |
...
| **IAM — Model** | **Granular AWS-style IAM**: Root + Users + Groups + Policies (Effect/Action/Resource/Condition) created by the tenant itself. Immutable policy versioning + audit log. (ADR 0007) Since US44 a user may also carry a **permission boundary**: one policy that CAPS them, so an action passes only if their own policies allow it and the boundary allows it (ADR 0049) | Does not impose a fixed role hierarchy (no predefined Manager/Analyst/Viewer). The boundary is not a hierarchy either: it attaches to a **user** and never to a group, the tenant **Root cannot be capped** by one, and a user without one is unrestricted rather than denied |
| **IAM — Conditions** | **AWS-style Conditions** on attributes defined by the tenant: `Department`, `Project`, `Account` etc. `PolicyEvaluator` supports `StringEquals`, `StringIn`, `StringLike`, `DateGreaterThan`, `DateLessThan`, and the listing endpoints evaluate them per item against the meeting's `attributes` (`V007`/`V008`) | Operators outside that list, and attributes missing from the context, result in `Deny` (fail-closed) — which is why, since US42, neither the policy form nor the JSON editor's schema will let one be written: an `Allow` carrying an unimplemented operator grants nothing at all. **No shipped UI writes those attributes**: neither the web upload nor the desktop sends an `attributes` map, so attribute-based conditions today require calling `POST /meetings` directly. The evaluator is real; the way to populate its input is not in the product yet — see §6 |
